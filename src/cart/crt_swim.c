			   ...
			   struct swim_member_state *state)
{
	 ...
	 * timeouts.
	 *
	 * If the rank is ALIVE with a higher incarnation, we mark
	 * corresponding epi objects but make no attempt to abort RPCs, because
	 * we currently can't determine whether the rank has restarted or just
	 * cleared a suspicion. We may improve this in the future, perhaps by
	 * clearing suspicions with only lower incarnation bits. Then, the
	 * crt_rank_mark_alive code will become unnecessary.
	 *
	 * Ignore the return values from the two calls; the function logs
	 * errors.
	 */
	if (cb_type == CRT_EVT_DEAD)
		crt_rank_epi_op(rank, CRT_RANK_EPI_MARK_DEAD_ABORT);
	else if (state_prev->sms_incarnation < state->sms_incarnation)
